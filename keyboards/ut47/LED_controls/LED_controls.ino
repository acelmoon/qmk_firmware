...
void checkserial() {
  if (Serial1.available() > 0) {
    iByte = Serial1.read();
    if (iByte == 100) { //Toggle OnOff
      if(mode==0){
        if(SaveState>1){
          brightness=SaveState;
          SaveState=1;
        }
        else{
          SaveState=brightness;
          brightness=1;
        }
      }
    }
    if (iByte == 101) { //Increase Brightness
      brightness++;
      if (brightness > 9) {
        brightness = 9;
      }
    }
    if(iByte==99){ //Decrease Brightness
      brightness--;
      if (brightness < 1) {
        brightness = 1;
      }
    }
    if(iByte==111){ //Mode Change
      mode++;
    }
    if(iByte==135){ //Default Mode
      mode=0;
      brightness=2;
    }
    if (iByte < 101) {
